...
function App() {
  ...
  return (
    <div className={`min-h-screen w-full flex flex-col transition-all duration-500 ${isDark ? "bg-gradient-to-br from-gray-900 via-slate-900 to-gray-800" : "bg-gradient-to-br from-sky-50 via-blue-50 to-indigo-50"}`}>
      ...
      {/* Footer */}
      <footer className={`border-t py-4 ${isDark ? "bg-slate-900/50 border-slate-700/50 text-gray-400" : "bg-white/50 border-sky-200/50 text-gray-500"}`} role="contentinfo">
        <div className="w-full px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-2 text-sm">
            <p>JWTBench &copy; {new Date().getFullYear()}</p>
            <div className="flex items-center gap-4">
              <a 
                href="https://dk.andiindia.in" 
                target="_blank" 
                rel="noopener noreferrer"
                className={`hover:underline transition-colors ${isDark ? "hover:text-white" : "hover:text-gray-900"}`}
              >
                Website
              </a>
              <a 
                href="https://github.com/thisisdkyadav/jwt" 
                target="_blank" 
                rel="noopener noreferrer"
                className={`hover:underline transition-colors ${isDark ? "hover:text-white" : "hover:text-gray-900"}`}
              >
