      ...
      <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8">
        ...
        <div className={`rounded-3xl p-6 sm:p-8 space-y-4 transition-all duration-300 ${isDark ? "bg-slate-800/50 backdrop-blur-xl border border-slate-700/50" : "bg-white/70 backdrop-blur-xl border border-sky-200/50 shadow-xl"}`}>
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className={`font-bold text-lg sm:text-xl tracking-wide ${isDark ? "text-white" : "text-gray-900"}`}>HMAC Secret</h3>
          </div>
          <input
        ...
        {secret && (
          <div className={`rounded-3xl p-6 sm:p-8 space-y-6 transition-all duration-300 ${isDark ? "bg-slate-800/50 backdrop-blur-xl border border-slate-700/50" : "bg-white/70 backdrop-blur-xl border border-sky-200/50 shadow-xl"}`}>
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className={`font-bold text-lg sm:text-xl tracking-wide ${isDark ? "text-white" : "text-gray-900"}`}>Security Analysis</h3>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {/* Strength */}
              <div className={`rounded-2xl p-6 transition-all duration-300 ${isDark ? "bg-slate-700/50 border border-slate-600/50" : "bg-white/50 border border-sky-200/50 shadow-inner"}`}>
                <div className={`text-sm font-medium mb-2 ${isDark ? "text-gray-300" : "text-gray-600"}`}>Strength Level</div>
                <div className={`text-2xl font-bold ${strength === "Strong" ? "text-emerald-500" : strength === "Medium" ? "text-amber-500" : "text-red-500"}`}>{strength}</div>
              ...
              {/* Length */}
              <div className={`rounded-2xl p-6 transition-all duration-300 ${isDark ? "bg-slate-700/50 border border-slate-600/50" : "bg-white/50 border border-sky-200/50 shadow-inner"}`}>
                <div className={`text-sm font-medium mb-2 ${isDark ? "text-gray-300" : "text-gray-600"}`}>Character Length</div>
                <div className={`text-2xl font-bold ${isDark ? "text-white" : "text-gray-900"}`}>{length}</div>
                <div className={`text-xs mt-2 ${isDark ? "text-gray-400" : "text-gray-500"}`}>{length < 8 ? "Too short" : length < 16 ? "Acceptable" : "Good length"}</div>
              </div>

              {/* Entropy */}
              <div className={`rounded-2xl p-6 transition-all duration-300 ${isDark ? "bg-slate-700/50 border border-slate-600/50" : "bg-white/50 border border-sky-200/50 shadow-inner"}`}>
                <div className={`text-sm font-medium mb-2 ${isDark ? "text-gray-300" : "text-gray-600"}`}>Entropy (bits)</div>
                <div className={`text-2xl font-bold ${isDark ? "text-white" : "text-gray-900"}`}>{entropy}</div>
            ...
            >
              <div className={`text-sm font-medium mb-2 ${strength === "Weak" ? "text-red-400" : strength === "Medium" ? "text-amber-400" : "text-emerald-400"}`}>Security Recommendations</div>
